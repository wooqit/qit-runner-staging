...
class PlaywrightResults extends Results {
	...
	private function get_debug_log(): string {
		...
		if ( file_exists( $file ) ) {
			...
			// Scrub absolute paths to avoid WAF blocking.
			return $this->scrub_paths( $log );
		}
	 ...
	 */
	private function scrub_paths( string $content ): string {
		// Common CI runner and Docker base paths to scrub.
		$paths_to_scrub = [
			...
			// Phar paths (WP-CLI).
			'phar://[^/]+/' => 'wp-cli/',
			// Escaped versions for JSON content.
			'\\\\/var\\\\/www\\\\/html\\\\/' => '',
			'\\\\/var\\\\/www\\\\/' => '',
			'phar:\\\\/\\\\/[^\\\\/]+\\\\/' => 'wp-cli/',
		];
	...
	public function send_results(): string {
		...
		return ( new RequestBuilder( $this->get_url(), $this->output ) )
			...
			->with_post_body( [
				...
				'test_result_json'          => $this->compress_field( $this->get_test_result_json() ),
				'test_result_json_original' => $this->get_test_result_json_original_compressed(),
				'ctrf_json'                 => $this->compress_field( $ctrf_json ),
				'debug_log'                 => $this->compress_field( $this->get_debug_log() ),
