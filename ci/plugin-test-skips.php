...
switch ( $qit_test_type ) {
	case 'woo-e2e':
			...
			'qit-skip' => [
				'all'                                         => [
					...
					'tests/product/product-images.spec.ts' => [
						'can set product image',
						'can update the product image',
						'can delete the product image',
						'can create a product gallery',
						'can update a product gallery',
					],
					'tests/email-editor/email-editor-settings-sidebar.spec.ts' => [
						'Can update email status',
						'Can update email subject and preview text',
						'Can update email recipients',
					],
				],
		...
		// This test requires wp-env and pnpm which are not available in QIT's CI environment.
		$data['qit-skip']['all']['tests/checkout/checkout-shortcode-custom-place-order-button.spec.ts'] = [
		...
		// These tests fail in QIT's CI environment.
		$data['qit-skip']['all']['tests/analytics/analytics-data.spec.ts'] = [
		...
		$data['qit-skip']['all']['tests/order/order-refund.spec.ts'] = [
			'can delete an issued refund',
		];

		$data['qit-skip']['all']['tests/product/product-import-csv.spec.ts'] = [
			'can upload the CSV file and import products',
			'can override the existing products via CSV import',
		];

		$data['qit-skip']['all']['tests/product/product-reviews.spec.ts'] = [
			'can reply to a product review',
		];

		$data['qit-skip']['all']['tests/onboarding/onboarding-wizard.spec.ts'] = [
			'Can complete the core profiler installing default extensions',
		];
