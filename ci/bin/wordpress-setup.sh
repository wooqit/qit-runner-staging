...
download_and_verify_wordpress() {
    ...
    # Skip checksum verification for pre-release WordPress versions (beta, RC, alpha, dev)
    # as these commonly have checksum mismatches with upstream.
    local skip_verify=false
    if [[ "$WORDPRESS_VERSION" =~ (beta|[rR][cC]|alpha|dev) ]]; then
        skip_verify=true
        echo "Pre-release WordPress version detected ($WORDPRESS_VERSION). Checksum verification will be skipped."
    fi

    # Skip checksum verification for WordPress 7.0+ due to a libzip path-length truncation
    # bug that causes files with relative paths longer than 90 characters to be extracted
    # with truncated names (e.g. wp-includes/php-ai-client deep paths introduced in WP 7.0).
    # TODO: Remove this skip once the libzip version in the Docker image is upgraded.
    if version_compare "$WORDPRESS_VERSION" "7.0" ">="; then
        skip_verify=true
        echo "WordPress 7.0+ detected ($WORDPRESS_VERSION). Checksum verification skipped pending libzip upgrade (see path-length truncation bug)."
    fi
    ...
    while (( attempts < max_attempts )); do
        ...
        if [ "$skip_verify" = true ]; then
            echo "Skipping checksum verification for pre-release version."
            return
        fi
...
for plugin in $(echo "${PLUGINS_JSON}" | jq -r '.[] | select(.type=="plugin") | .slug'); do
  # Set WooCommerce Subscriptions gifting announcement as dismissed
  if [ "$plugin" == "woocommerce-subscriptions" ]; then
    echo "Setting WooCommerce Subscriptions gifting welcome announcement as dismissed"
    docker_wp "wp option update woocommerce_subscriptions_gifting_is_welcome_announcement_dismissed 1"
  fi
