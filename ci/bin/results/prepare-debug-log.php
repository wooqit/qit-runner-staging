...
if ( file_exists( $debug_log_file ) ) {
	...
	while ( $debug_log->valid() ) {
		...
		if ( preg_match( '/^\[.*\]\s+(.*)$/', $line, $matches ) ) {
			...
			// Ignore translation loading for the woocommerce domain was triggered too early, if not testing Woo.
			$is_woocommerce_load_textdomain = stripos( $current_entry, 'Translation loading for the woocommerce domain was triggered too early.' ) !== false;

			if ( $is_woocommerce_load_textdomain && ! $is_testing_woocommerce_core && ! $has_sut_slug_in_error ) {
				$skip_current_entry = true;
			}

			// If not testing Woo Core, ignore "WCAdminHelper::is_store_page is deprecated since version 9.8.0".
			// @see https://github.com/woocommerce/woocommerce/pull/57169
			$is_wcadminhelper_is_store_page = stripos( $current_entry, 'WCAdminHelper::is_store_page is deprecated since version 9.8.0' ) !== false;

			if ( $is_wcadminhelper_is_store_page && ! $is_testing_woocommerce_core && ! $has_sut_slug_in_error ) {
				$skip_current_entry = true;
			}

			// If not testing Woo Core, ignore "table doesn't exist" DB errors for WooCommerce core tables that
			// WooCommerce queries during its own activation, before WC_Install creates the tables. These are
			// WooCommerce-core false positives (the query is Woo's own, about Woo's own table), not SUT bugs:
			//   - wp_wc_tax_rate_classes: queried via tax settings during boot.
			//   - wp_woocommerce_attribute_taxonomies: queried by VisualAttributeTermAdmin (@since WC 10.9.0)
			...
			if (
				! $is_testing_woocommerce_core &&
				! $has_sut_slug_in_error &&
				stripos( $current_entry, 'WordPress database error' ) !== false &&
				stripos( $current_entry, 'exist for query' ) !== false
			) {
				foreach ( $woo_core_activation_tables as $woo_core_table ) {
					if ( stripos( $current_entry, $woo_core_table ) !== false ) {
						$skip_current_entry = true;
						break;
					}
				}
			 ...
			 */
			if (
				stripos( $current_entry, 'could not establish a secure connection to WordPress.org' ) !== false
				|| stripos( $current_entry, 'Something may be wrong with WordPress.org' ) !== false
			) {
				$skip_current_entry = true;
			}
	...
	foreach ( $log_entries as $hash => $entry ) {
		if (
			stripos( $entry['message'], 'You have an error in your SQL syntax' ) !== false
			&& stripos( $entry['message'], 'wc_delete_related_product_transients_async' ) !== false
		) {
			$log_entries[ $hash ]['count'] = 1;
		}
	}
