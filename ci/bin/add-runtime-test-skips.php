			...
			foreach ( $plugins as $plugin => $files ) {
				...
				foreach ( $files as $file => $tests ) {
					...
					if ( ! file_exists( $file_path ) ) {
						// WooCommerce 10.7.0+ migrated test files from .js to .ts.
						// Try the alternate extension before giving up.
						$alt_path = preg_replace( '/\.(js|ts)$/', '', $file_path );
						$matches  = glob( $alt_path . '.{js,ts}', GLOB_BRACE );
						if ( ! empty( $matches ) ) {
							$file_path = $matches[0];
						} else {
					...
					foreach ( $tests as $test ) {
						...
						if ( $remove ) {
							...
							$escaped_test  = preg_quote( $test, '/' );
							$pattern       = "/(['\"]\s*)$escaped_test(\s*['\"])(,)/i";
