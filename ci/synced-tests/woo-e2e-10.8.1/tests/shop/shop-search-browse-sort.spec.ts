		...
		test( 'should let user sort the products in the shop', async ( {
			...
			await test.step( 'Go to the shop and sort by price high to low', async () => {
				...
				const highToLowList = await page
					...
					.getByRole( 'heading' )
					.allInnerTexts();
				for ( let shopPage = 2; shopPage <= 10; shopPage++ ) {
                    const response = await page.goto( 'shop/page/' + shopPage + '/?orderby=price-desc' );
                    if ( ! response || response.status() === 404 ) {
                        break;
                    }
                    const pageProductNames = await page.getByRole( 'listitem' ).getByRole( 'heading' ).allInnerTexts();
                    if ( pageProductNames.length === 0 ) {
                        break;
                    }
                    highToLowList.push( ...pageProductNames );
                }
				...
				expect( highToLow_index_priciest ).toBeGreaterThan( -1 );
                expect( highToLow_index_cheapest ).toBeGreaterThan( -1 );
			...
			await test.step( 'Go to the shop and sort by price low to high', async () => {
				...
				const lowToHighList = await page
					...
					.allInnerTexts();
				for ( let shopPage = 2; shopPage <= 10; shopPage++ ) {
                    const response = await page.goto( 'shop/page/' + shopPage + '/?orderby=price' );
                    if ( ! response || response.status() === 404 ) {
                        break;
                    }
                    const pageProductNames = await page.getByRole( 'listitem' ).getByRole( 'heading' ).allInnerTexts();
                    ...
                    lowToHighList.push( ...pageProductNames );
                }
				...
				expect( lowToHigh_index_priciest ).toBeGreaterThan( -1 );
                expect( lowToHigh_index_cheapest ).toBeGreaterThan( -1 );
