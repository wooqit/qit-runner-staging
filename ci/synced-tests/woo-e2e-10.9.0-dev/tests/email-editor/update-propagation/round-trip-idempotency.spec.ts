...
test.describe( 'Update propagation — round-trip and idempotency', () => {
	 ...
	 */
	test.skip( 'Auto-apply round-trip: uncustomized post returns to in_sync', async () => {
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'Selective apply round-trip: edit, bump, apply with keep-yours → stays customized', async () => {
		const oldHtml =
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'Reset round-trip: customized → reset → in_sync', async () => {
		const customized =
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'Detection sweep is idempotent: second run touches zero posts', async () => {
		await simulateCoreBump( 'new_order', OLD_HTML );
		const postId = await seedWooEmailPost( {
			emailId: 'new_order',
