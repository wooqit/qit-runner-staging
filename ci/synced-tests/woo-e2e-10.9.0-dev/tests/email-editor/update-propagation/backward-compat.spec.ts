...
test.describe( 'Update propagation — backward compatibility', () => {
	 ...
	 */
	test.skip( 'BC Case A — content matches current core, no stamp meta', async () => {
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'BC Case B — timestamps equal and content behind core', async () => {
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( '@pr BC Case C — customized post content preserved (critical safety)', async () => {
		const customized =
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'BC no mass-fire on first upgrade: zero _available, one _backfill_completed', async ( {
		page,
	} ) => {
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( 'BC migration is idempotent: second backfill is a no-op', async () => {
		const postId = await seedWooEmailPost( {
			emailId: 'new_order',
			stripStampMeta: true,
		} );
