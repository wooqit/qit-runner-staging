...
test.describe( 'Update propagation — core flows', () => {
	 ...
	 */
	test.skip( '@pr Plugin update triggers divergence detection and classifies posts', async () => {
	 ...
	 *   "Show browser" eye: ON.
	 */
	test.skip( '@pr Update-available indicator appears on email list and in editor', async ( {
		page,
	} ) => {
		await simulateCoreBump( 'new_order', OLD_HTML );
	 ...
	 *   "Show browser" eye: ON.
	 */
	test.skip( '@pr Auto-apply succeeds silently for unmodified posts', async ( {
		page,
	} ) => {
	 ...
	 *   "Show browser" eye: not needed.
	 */
	test.skip( '@pr Selective apply succeeds and preserves customizations', async () => {
		const oldHtml =
	 ...
	 *   "Show browser" eye: ON.
	 */
	test.skip( '@pr Dismiss flow records the dismissed Tracks event', async ( {
		page,
	} ) => {
	 ...
	 *   "Show browser" eye: ON.
	 */
	test.skip( 'Review drawer: pick per-conflict yours vs core and apply', async ( {
		page,
	} ) => {
