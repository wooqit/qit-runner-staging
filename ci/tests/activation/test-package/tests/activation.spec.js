...
test('Create a Simple Order', { tag: [ '@basic' ] }, async ({ page }) => {
    ...
    await page
        ...
        .click();
    // WooCommerce opens the product search as the modal initialises, so there is
    // nothing to click here. selectWoo also keeps a zero-size search field inside
    // the selection itself; scoping to the open container picks the one appended
    // to <body>, which is the one a merchant types into.
    await page
