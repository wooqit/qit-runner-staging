...
class QIT_Validation_Command {
	...
	public function __invoke( $args, $assoc_args ) {
		...
		$required_header_map = array_flip( $required_headers );
		$file_data           = $this->get_extension_metadata( $type, $slug, $sut_directory, $sut_entry_point, $required_header_map );
		$readme_data         = $this->get_readme_metadata( $type, $slug, $sut_directory, $required_header_map );
		...
		if ( ! empty( $data ) ) {
			...
			if ( $type === 'plugin' && class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
				...
				$results['features'] = \Automattic\WooCommerce\Utilities\FeaturesUtil::get_compatible_features_for_plugin( $sut_directory . '/' . $sut_entry_point );
			}
	...
	protected function get_extension_metadata( $type, $slug, $sut_directory, $sut_entry_point, array $headers ): array {
		$data = [];
		...
		if ( $type === 'plugin' ) {
			...
			$style_css_candidates = [
				WP_CONTENT_DIR . '/themes/' . $sut_directory . '/style.css',
				WP_CONTENT_DIR . '/themes/' . $slug . '/style.css',
			];

			foreach ( array_unique( $style_css_candidates ) as $style_css_file ) {
				if ( file_exists( $style_css_file ) ) {
					$data = get_file_data( $style_css_file, $headers, $type );
					break;
				}
			}
		}
	...
	protected function get_readme_file( $type, $slug, $sut_directory ) {
		if ( $type === 'plugin' ) {
			$dir = WP_PLUGIN_DIR . '/' . $sut_directory;
		} elseif ( $type === 'theme' ) {
			$dir = WP_CONTENT_DIR . '/themes/' . $sut_directory;
		} else {
			return '';
		}

		// Try common casings since Linux filesystems are case-sensitive.
		foreach ( [ 'readme.txt', 'README.txt', 'README.TXT' ] as $filename ) {
			$path = $dir . '/' . $filename;
			if ( file_exists( $path ) ) {
				return $path;
			}
