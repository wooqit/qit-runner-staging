...
class Compatibility_Test {
	...
	protected function run_stack_smoke_tests(): void {
		$stack_slugs = array_keys( $this->extensions_to_test );
		$stack_label = empty( $stack_slugs ) ? $this->main_sut_slug : implode( ',', $stack_slugs );

		echo "Running compatibility stack smoke for: $stack_label\n";

		echo $this->exec_in_container( sprintf( 'wp option add cd_qty_extensions_being_tested "%s"', count( $this->extensions_to_test ) ), false ) . PHP_EOL;
		echo $this->exec_in_container( 'wp option add cd_enable_error_logging 1', false ) . PHP_EOL;
		echo $this->exec_in_container( sprintf( 'wp option add qit_main_sut_slug "%s"', str_replace( '"', '', $this->main_sut_slug ) ), false ) . PHP_EOL;
		echo $this->exec_in_container( sprintf( 'wp option update cd_activated_alongside "%s"', addcslashes( $stack_label, '"' ) ), false ) . PHP_EOL;

		create_mimick_server_script( __DIR__ . '/go-to-frontpage.php' );
		create_mimick_server_script( __DIR__ . '/go-to-cart.php', 'cart' );
		create_mimick_server_script( __DIR__ . '/go-to-checkout.php', 'checkout' );
		create_mimick_server_script( __DIR__ . '/go-to-account.php', 'my-account' );
		create_mimick_server_script( __DIR__ . '/go-to-admin.php', 'wp-admin' );
		create_mimick_server_script( __DIR__ . '/go-to-rest.php', 'wp-json' );

		if ( array_key_exists( 'woocommerce', $this->extensions_to_test ) ) {
			echo "Activating WooCommerce first.\n";
			$this->exec_in_container_and_log_stderr( 'wp plugin activate "woocommerce"' );
			unset( $this->extensions_to_test['woocommerce'] );
		}

		foreach ( $this->extensions_to_test as $plugin_slug => $plugin_dir_path ) {
			echo "\n== Activating $plugin_slug from $plugin_dir_path ==\n";

			if ( $this->main_sut_type === 'theme' && $plugin_slug === $this->main_sut_slug ) {
				$this->exec_in_container_and_log_stderr( sprintf( 'wp theme activate "%s"', addcslashes( $plugin_slug, '"' ) ) );
				continue;
			}
		...
		$probes = [
			'Front page' => 'php ' . __DIR__ . '/go-to-frontpage.php',
			'Cart'       => 'php ' . __DIR__ . '/go-to-cart.php',
			'Checkout'   => 'php ' . __DIR__ . '/go-to-checkout.php',
			'Account'    => 'php ' . __DIR__ . '/go-to-account.php',
			'Admin'      => 'php ' . __DIR__ . '/go-to-admin.php',
			'REST'       => 'php ' . __DIR__ . '/go-to-rest.php',
		];

		foreach ( $probes as $label => $command ) {
			echo "Probing $label...\n";
			$this->exec_in_container_and_log_stderr( $command );
		}

		echo $this->exec_in_container( "du -h /var/www/html/wp-content/mu-plugins/compatibility-results.txt /var/www/html/wp-content/mu-plugins/cd-stderr.txt && wc -l /var/www/html/wp-content/mu-plugins/compatibility-results.txt", false ) . PHP_EOL;
		echo $this->exec_in_container( sprintf( 'php -r "%s"', "die(is_null(json_decode(file_get_contents('/var/www/html/wp-content/mu-plugins/compatibility-results.txt'), true)) ? 1 : 0);" ), false ) . PHP_EOL;
