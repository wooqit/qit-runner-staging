...
// Temporarily disabled. Asserts the fix from WooCommerce #67006, which ships
// in 11.1. The environment installs the stable channel (11.0.1), where the
// "Save zone and method" button does not exist, so this fails on every run.
// Re-enable once the WooCommerce version gate lands.
test.skip( 'saves an unsaved shipping zone when adding a method', async ( {
	page,
	restApi,
} ) => {
