...
test.describe( 'Edit order', { tag: [ tags.SERVICES, tags.HPOS ] }, () => {
	...
	test( 'can copy billing address to shipping address', async ( {
		...
		// Temporarily skipped. Asserts the fix from WooCommerce #66381, which
		// ships in 11.1. The environment installs the stable channel (11.0.1),
		// where the name field stays 'Shipping' whatever method is chosen, so
		// this step fails on every run. The rest of the test behaves the same on
		// both versions. Re-enable once the WooCommerce version gate lands.
		await test.step.skip( 'Update the shipping method name', async () => {
