...
test.describe(
	...
	() => {
		...
		// Temporarily disabled. Asserts the fix from WooCommerce #66343, which
		// ships in 11.1. The environment installs the stable channel (11.0.1),
		// where Enter still submits and closes the modal, so this fails on every
		// run. Re-enable once the WooCommerce version gate lands.
		test.skip( 'can add a product using the keyboard without a rogue search box', async ( {
			page,
			simpleProduct,
		} ) => {
			await page.goto( 'wp-admin/admin.php?page=wc-orders&action=new' );
