...
## Which WooCommerce this tracks

This directory on trunk tracks WooCommerce trunk, and the `Publish Woo E2E Test
Package` workflow publishes it as `woocommerce/core-e2e-tests:nightly` on every
push to trunk that touches `test-package/`. That is what `--woo=nightly` and an
unreleased version ahead of every published line run. Released lines run the
two-segment versions (`11.2`), published by hand from `woo-core-tests/<x.y>`
branches, and `latest` is republished by hand from the newest stable line's
branch. So a spec taken from upstream trunk belongs here; a fix for a released
line belongs on its branch.
