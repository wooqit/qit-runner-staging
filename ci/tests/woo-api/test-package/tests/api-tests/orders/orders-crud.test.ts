...
test.describe.serial( 'Orders API tests: CRUD', () => {
	...
	test.describe( 'Update an order', () => {
		...
		test( 'can add shipping and billing contacts to an order', async ( {
			...
			const responseJSON = await response.json();
			expect( response.status() ).toEqual( 200 );
			expect( responseJSON.billing ).toEqual(
				expect.objectContaining( updatedCustomerBilling )
			);
			expect( responseJSON.shipping ).toEqual(
				expect.objectContaining( updatedCustomerShipping )
			);
		} );
