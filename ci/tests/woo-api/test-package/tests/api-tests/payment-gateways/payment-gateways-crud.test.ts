	...
	test( 'can view all payment gateways', async ( { request } ) => {
		...
		expect( codSettings ).toEqual(
			expect.objectContaining( {
				...
				enable_for_methods: expect.objectContaining( {
					id: 'enable_for_methods',
					label: 'Enable for shipping methods',
					description:
						'If Cash on delivery is only available for certain methods, set it up here. Leave blank to enable for all methods.',
					type: 'multiselect',
					value: '',
					default: '',
					tip: 'If Cash on delivery is only available for certain methods, set it up here. Leave blank to enable for all methods.',
					placeholder: '',
				...
				enable_for_virtual: expect.objectContaining( {
