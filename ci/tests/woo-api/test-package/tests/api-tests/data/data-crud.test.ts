...
test.describe( 'Data API tests', () => {
	...
	test.skip( 'can view country data', async ( { request } ) => {
		...
		expect( responseJSON ).toEqual(
			expect.arrayContaining( [
				expect.objectContaining( {
					code: 'MOP',
					name: 'Macanese pataca',
					// WooCommerce corrected this symbol in 11.1
					// (woocommerce/woocommerce#66716). QIT runs against a range
					// of WooCommerce versions, so accept both spellings.
					symbol: expect.stringMatching( /^(P|MOP&#36;)$/ ),
					_links: {
						self: [
							{
								href: expect.stringContaining(
		...
		expect( responseJSON ).toEqual(
			expect.arrayContaining( [
				expect.objectContaining( {
					...
					name: 'Zambian kwacha',
					// WooCommerce corrected this symbol in 11.1
					// (woocommerce/woocommerce#66722). QIT runs against a range
					// of WooCommerce versions, so accept both spellings.
					symbol: expect.stringMatching( /^(ZK|K)$/ ),
					_links: {
						self: [
							{
								href: expect.stringContaining(
